                        ...
                        <table id="ecommerce-category-list" class="table  table-bordered">
                            ...
                            <tbody>
                                @foreach($categories as $category)
                                    <tr>
                                        <td>
                                            <img style="width: 60px;" src="{{ $category->photo ? asset('assets/images/'.$category->photo) : asset('assets/images/placeholder.png') }}" alt="Image Not Found">
                                        </td>
                                        <td>
                                            {{ $category->name }}
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <div class="dropdown">
                                                <button class="btn btn-{{  $category->status == 1 ? 'success' : 'danger'  }} btn-sm  dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                {{  $category->status == 1 ? __('Enabled') : __('Disabled')  }}
                                                </button>
                                                <div class="dropdown-menu animated--fade-in" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item" href="{{ route('category.status',[$category->id,1]) }}">{{ __('Enable') }}</a>
                                                <a class="dropdown-item" href="{{ route('category.status',[$category->id,0]) }}">{{ __('Disable') }}</a>
                                                </div>
                                            </div>
                                    ...
                                    <div class="modal fade" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="confirm-deleteModalLabel" aria-hidden="true">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form action="{{ route('category.destroy',$category->id) }}" class="d-inline btn-ok" method="POST">
